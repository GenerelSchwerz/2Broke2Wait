...
export class WebhookReporter extends ProxyServerPlugin<AllOpts, AllEvents> {
  ...
  private sendOrEdit = async (config: WebhookWrapper, message: any) => {
    if (config.edit) {
      if (config.firstMessageId == null) {
        const msg = await this.queueInfo.client.send(message);
        config.firstMessageId = msg.id;
        return msg;
      } else {
        return await this.queueInfo.client.editMessage(config.firstMessageId, message);
      }
    } else {
      return await this.queueInfo.client.send(message)
    }
  };
