...
> {
  ...
  private readonly listenerMap: Map<keyof Emits, Array<{ original: Function, ref: Function }>> = new Map()
   ...
   * @returns void
   */
  public serverOn<Key extends keyof Emits>(event: Key, listener: Emits[Key] extends Function ? Emits[Key] : never) {
    ...
    if (listeners.findIndex((check) => check.original === test) > -1) { throw Error(`Registering event ${String(event)} twice on ${this.constructor.name}`) }
   ...
   * @returns void
   */
  public serverOff<Key extends keyof Emits>(event: Key, listener: Emits[Key] extends Function ? Emits[Key] : never) {
    const listeners = this.listenerMap.get(event)
   ...
   * @param server
   */
  public onLoad (server: ProxyServer<Opts, Emits>) {
    this._server = server
   ...
   * @param server
   */
  public onUnload (server: ProxyServer<Opts, Emits>) {
    this._server = server
   ...
   * @param args
   */
  public serverEmit<E extends keyof Emits>(event: E, ...args: Arguments<Emits[E]>) {
...
export class ServerBuilder<Opts extends IProxyServerOpts, Events extends IProxyServerEvents, AppliedSettings = false> {
  ...
  public build<This extends ServerBuilder<Opts, Events, true>>(this: This) {
    ...
    return srv as unknown as ProxyServer<Opts, Events>
  }
}
  ...
  constructor (lsOpts: ServerOptions, psOpts: Opts, bOpts: BotOptions, other: OtherProxyOpts = {}) {
    ...
    this.emit = (event: any, ...args: any[]) => {
      ...
      if (this.otherOpts.debug) {
        if (typeof event === 'string' && event.startsWith('botevent_')) return // handled elsewhere;
