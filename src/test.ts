...
import { AntiAFKEvents, AntiAFKOpts, StrictAntiAFKEvents } from "./impls/antiAfkServer";
import merge from "ts-deepmerge"

import type { Bot, BotOptions } from "mineflayer"
import type { Client, Server, createServer, ServerClient, PacketMeta } from "minecraft-protocol";
import { Conn } from "@rob9315/mcproxy";
import { sleep } from "./util/index";


class ProxyEmitter extends (EventEmitter2 as { new(): StrictEventEmitter<EventEmitter2, IProxyServerEvents> }) { }

class AntiAFKEmitter extends (EventEmitter2 as { new(): StrictEventEmitter<EventEmitter2, StrictAntiAFKEvents> }) { }

type GConstructor<T = {}> = new (...args: any[]) => T;

type BasicEmitter<T> = GConstructor<StrictEventEmitter<EventEmitter2, T>>;


function BuildProxyBase<Opts extends IProxyServerOpts, T extends BasicEmitter<IProxyServerEvents> = BasicEmitter<IProxyServerEvents>>(Base: T) {
    ...
    return class extends Base {
         ...
         */
        protected optionValidation = () => { return this.psOpts };
...
const ProxyServer = BuildProxyBase<AntiAFKOpts>(ProxyEmitter);
type TProxyServer = typeof ProxyServer.prototype;
