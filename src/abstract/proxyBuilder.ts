...
import { IProxyServerEvents, IProxyServerOpts } from "./proxyServer";
...
export function BuildProxyBase<
  ...
  Key extends IProxyServerEvents = IProxyServerEvents
>(Base: BasicEmitter<Key>) {
  return class ProxyBase extends Base {
     ...
     * @param {boolean} onlineMode Whether or not to destroy internal server on close.
     * @param {BotOptions} bOpts Bot options for the proxy.
    ...
    public closeConnections = (reason: string = "Proxy stopped.") => {
      ...
      this.emit("decidedClose" as any, reason);
    };
...
class ProxyEmitter extends (EventEmitter2 as {
  new (): StrictEventEmitter<EventEmitter2, IProxyServerEvents>;
}) {}
