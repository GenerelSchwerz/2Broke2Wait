...
import { ClientEventRegister, QueueEventRegister, ServerEventRegister } from "../abstract/eventRegisters";
...
> {
  ...
  protected buildServerEmbed() {
    ...
    const embed: APIEmbed = {
      ...
      footer: {
        text:
          (this.srv.isProxyConnected()
            ? `Connected to: ${this.srv.bOpts.host}${this.srv.bOpts.port !== 25565 ? ":" + this.srv.bOpts.port : ""}\n`
            : "Not connected.\n") +
          (this.srv.controllingPlayer != null ? `Connected player: ${this.srv.controllingPlayer.username}\n` : "") +
          (this.srv.queue?.inQueue && !(eta == null)
            ? `Join time: ${DateTime.local().plus(eta).toFormat("hh:mm a MM/dd/yyyy")}\n`
            : ""),
      },
    };
    return embed;
