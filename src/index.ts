  ...
  switch (inp.split(" ")[0]) {
    ...
    case "status":
      if (afkServer.isProxyConnected()) {
        console.log(
          `Proxy connected to ${afkServer.bOpts.host}:${afkServer.bOpts.port !== 25565 ? afkServer.bOpts.port : ""}`
        );
        if (afkServer.queue.inQueue) console.log(`Proxy in queue? ${afkServer.queue.inQueue}`);
      } else {
