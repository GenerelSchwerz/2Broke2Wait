...
const { Client, discord, Intents, MessageEmbed } = require("discord.js");
...
try {
  ...
  if (String(err).includes("SyntaxError: ")) {
    console.error(
      "The syntax in your config file is not correct. Make sure you replaced all values as the README says under 'How to Install' step 5. If it still does not work, check that all quotes are closed. You can look up the json syntax online. Please note that the comments are no problem although comments are normally not allowed in json. " +
        err
    );
    process.exit(1);
  }
...
const askForSecrets = async () => {
  ...
  if (
    !(
      config.has("username") &&
      config.has("mcPassword") &&
      config.has("updatemessage")
    )
  ) {
    canSave = true;
    accountType =
      (await promisedQuestion(
        "Account type, mojang (1) or microsoft (2) [1]: "
      )) === "2"
        ? "microsoft"
        : "mojang";
  ...
  if (
    (!config.has("discordBot") || config.get("discordBot")) &&
    !config.has("BotToken")
  ) {
    canSave = true;
    discordBotToken = await promisedQuestion(
      "BotToken, leave blank if not using discord []: "
    );
    localConf.BotToken = discordBotToken;
  }
  localConf.discordBot =
    discordBotToken === ""
      ? false
      : config.has("discordBot") && config.get("discordBot");

  if (canSave) {
    savelogin = await promisedQuestion(
      "Save login for later use? Y or N [N]: "
    );
  ...
  if (localConf.discordBot) {
    ...
    dc.login(discordBotToken ?? config.get("BotToken")).catch(() => {
      console.warn(
        "There was an error when trying to log in using the provided Discord bot token. If you didn't enter a token this message will go away the next time you run this program!"
      ); //handle wrong tokens gracefully
    });
    dc.on("ready", () => {
  ...
  console.log(
    `Finished setting up 2b2w. Type "Start" to start the queue. Type "Help" for the list of commands.`
  );
  cmdInput();
  joinOnStart();
};
...
function startAntiAntiAFK() {
  if (!config.has("antiAntiAFK.enabled") || !config.get("antiAntiAFK.enabled"))
    return;
...
function join() {
      ...
      case "playerlist_header":
        if (!finishedQueue && true) {
          ...
          if (
            ...
            server.favicon = config.has("favicon") ? config.get("favicon") : ""; // fuck the icon
            server.motd = `Place in queue: ${webserver.queuePlace} ETA: ${webserver.ETA}`; // set the MOTD because why not
            webserver.ETA =
              Math.floor(ETAmin / 60) + "h " + Math.floor(ETAmin % 60) + "m";
            ...
            if (
              config.get("notification.enabled") &&
              positioninqueue <= config.get("notification.queuePlace") &&
              !notisend &&
              config.discordBot &&
              dcUser != null
            ) {
              sendDiscordMsg(
                dcUser,
                "Queue",
                "The queue is almost finished. You are in Position: " +
                  webserver.queuePlace
              );
              notisend = true;
            }
          }
        ...
        if (finishedQueue === false) {
          ...
          if (chatMessage == "Connected to the server.") {
            if (config.get("expandQueueData")) {
              ...
              let timeQueueTook =
                DateTime.local().toSeconds() - queueStartTime.toSeconds();
              let b = Math.pow(
                (0 + c) / (queueStartPlace + c),
                1 / timeQueueTook
              );
  ...
  const onDisconnect = () => {
    ...
    if (!stoppedByPlayer) {
      ...
      if (!config.has("MCpassword") && !config.has("password"))
        log(
          "If this ^^ message shows up repeatedly, it is likely a problem with your token being invalidated. Please start minecraft manually or use credential authentication instead."
        );
    }
...
function userInput(cmd, DiscordOrigin, discordMsg, channel) {
  ...
  switch (cmd) {
    ...
    case "stats":
      try {
        if (conn.bot.health == undefined && conn.bot.food == undefined) {
          ...
          if (conn.bot.health == 0) console.log("Health: DEAD");
          else console.log("Health: " + Math.ceil(conn.bot.health) / 2 + "/10");
          if (conn.bot.food == 0) console.log("Hunger: STARVING");
          else console.log("Hunger: " + conn.bot.food / 2 + "/10");
        }
      } catch (err) {
    ...
    case "loop status":
      ...
      else console.log("Loop is disabled");
      break;
    case "loop enable":
      if (JSON.stringify(webserver.restartQueue) == "true")
      ...
      switch (doing) {
        case "queue":
          msg(
            ...
            discordMsg,
            "Reconnecting",
            `Position: ${webserver.queuePlace} \n Estimated time until login: ${webserver.ETA}`
          );
          console.log(
            "Position: " +
              webserver.queuePlace +
              "  Estimated time until login: " +
              webserver.ETA
          );
          break;
        case "timedStart":
          msg(
            ...
            discordMsg,
            "Timer",
            "Timer is set to " + starttimestring
          );
          break;
        case "reconnect":
          msg(
            ...
            discordMsg,
            "Reconnecting",
            "2b2t is currently offline. Trying to reconnect"
          );
          break;
        case "auth":
        ...
        case "calcTime":
          let calcMsg = msg(
            ...
            discordMsg,
            "Calculating time",
            "Calculating the time, so you can play at " + starttimestring
          );
          break;
      }
      break;
    ...
    default:
      if (/start (\d|[0-1]\d|2[0-3]):[0-5]\d$/.test(cmd)) {
        doing = "timedStart";
        timedStart = setTimeout(
          startQueuing,
          timeStringtoDateTime(cmd).toMillis() - DateTime.local().toMillis()
        );
        ...
        msg(
          ...
          discordMsg,
          "Timer",
          "Queue is starting at " + starttimestring
        );
        ...
        msg(
          ...
          discordMsg,
          "Time calculator",
          "The perfect time to start the queue will be calculated, so you can play at " +
            starttimestring
        );
...
function sendDiscordMsg(channel, title, content) {
  ...
  channel
    ...
    .catch(() => {
      console.warn(
        `There was a permission error! Please make sure your bot has perms to talk.`
      ); //handle wrong tokens gracefully
    });
}
...
function calcTime(msg) {
  https
    .get("https://2b2t.io/api/queue", function (res) {
      doing = "calcTime";
        ...
        https
          .get("https://2b2t.io/api/queue", (resp) => {
            ...
            resp.on("end", () => {
              ...
              if (
                playTime.toSeconds() - DateTime.local().toSeconds() <
                waitTime
              ) {
                startQueuing();
                clearInterval(interval.calc);
    ...
    .on("error", function (e) {
      console.log(
        `2b2t.io is currently offline. Please try again later to use the "play" command.`
      );
    });
}
...
process.on("uncaughtException", (err) => {
  ...
  console.log(
    boxen(
      `Something went wrong! Feel free to contact us on discord or github! \n\n Github: https://github.com/themoonisacheese/2bored2wait \n\n Discord: https://discord.next-gen.dev/`,
      {
